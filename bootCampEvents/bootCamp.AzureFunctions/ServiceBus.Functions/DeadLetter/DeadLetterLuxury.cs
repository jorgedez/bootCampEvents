...
{
    public static class DeadLetterLuxury
    {
        private static readonly TelemetryClient _telemetry = new TelemetryClient();
        private const string functionName = "DeadLetterLuxury";

        [FunctionName(functionName)]
        public static void Run([ServiceBusTrigger(QueueNames.LuxuryDeadLetter, AccessRights.Manage, Connection = "AzureWebJobsServiceBus")]string sbMessage, TraceWriter log)
        {
            LoggerHelper.WriteTrace(functionName, $"Atención se ha enviado a {QueueNames.LuxuryDeadLetter} | {sbMessage} | a las {DateTime.UtcNow.ToString("dd/MM/yyyy HH-mm-ss")}", log, TraceLevel.Info, _telemetry);
