...
{
    public static class DeadLetterLowCost
    {
        private static readonly TelemetryClient _telemetry = new TelemetryClient();
        private const string functionName = "DeadLetterLowCost";

        [FunctionName(functionName)]
        public static void Run([ServiceBusTrigger(QueueNames.LowCostDeadLetter, AccessRights.Manage, Connection = "AzureWebJobsServiceBus")]string sbMessage, TraceWriter log)
        {
            LoggerHelper.WriteTrace(functionName, $"Atención se ha enviado a {QueueNames.LowCostDeadLetter} | {sbMessage} | a las {DateTime.UtcNow.ToString("dd/MM/yyyy HH-mm-ss")}", log, TraceLevel.Info, _telemetry);
