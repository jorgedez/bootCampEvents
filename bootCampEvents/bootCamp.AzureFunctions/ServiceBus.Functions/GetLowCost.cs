...
{
    public static class GetLowCost
    {
        private static TelemetryClient _telemetry = new TelemetryClient();
        private const string functionName = "GetLowCost";

        [FunctionName(functionName)]
        public static void Run([ServiceBusTrigger(QueueNames.LowCost, AccessRights.Manage, Connection = "AzureWebJobsServiceBus")]string sbMessage, TraceWriter log)
        {
            try
            {
